...
```bash
go run ./cmd/music-lib-web --addr 127.0.0.1:51991 --download-dir ./Downloads --concurrency 3
```
...
- 网易云 Cookie 是可选项。填写后会尝试使用账号可用的更高音质下载；Cookie 只保存在本地服务内存中，后续任务会复用，不会保存到文件或 API 响应，重启服务后失效。
