...
- 默认请求无损；无损不可用时会自动降级为 MP3，也可以手动选择只下载 MP3。
...
3. 可选：填写下载路径、Cookie，并选择无损或 MP3。默认会先尝试无损，失败后自动降级 MP3。
...
- 默认会先请求 FLAC 等无损资源，通常需要 Cookie 对应账号具备权限。
- 如果无损地址获取失败，或无损地址可用但实际下载失败，会自动改为 MP3 再试一次。
- 手动选择 MP3 时，会直接请求标准音质。
...
curl -X POST http://127.0.0.1:51873/api/jobs \
  ...
  -d '{"playlist_link":"https://music.163.com/#/playlist?id=123456","quality":"lossless"}'
```
